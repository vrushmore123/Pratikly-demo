...
const Matters = () => {
  ...
  // Function to update SVG connections based on current node positions// Function to update SVG connections based on current node positions
const updateConnections = () => {
  ...
  return (
    <section 
      ref={ref} 
      className="relative px-6 py-20 md:py-32 bg-white" 
      id="our-approach"
    >
      <div className="max-w-7xl mx-auto relative z-10">
        <div className={`flex flex-col items-center text-center mb-16 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="mb-4">
            <span className="inline-block px-5 py-2 bg-gradient-to-r from-blue-50 to-blue-100 text-blue-700 rounded-full text-sm font-medium shadow-sm transition-transform hover:scale-105 hover:shadow-md">
              {translations[language].title}
            </span>
          </div>
          <h2 className="text-4xl md:text-6xl font-bold leading-tight mb-6 max-w-4xl bg-clip-text text-transparent bg-gradient-to-r from-blue-700 via-indigo-600 to-purple-700">
            {translations[language].subtitle}
          </h2>
          <p className="text-xl md:text-2xl mb-8 max-w-3xl text-gray-700">
            {translations[language].description}
          </p>
          
          {/* Tab selection with animated indicator */}
          <div className="relative p-1 bg-gray-100 rounded-full mb-16 shadow-inner">
            <div 
              className="absolute inset-y-1 rounded-full bg-white shadow-md transition-all duration-300"
              style={{ 
            ...
            <div className="relative flex space-x-2">
              <button 
                className={`relative z-10 px-8 py-3 rounded-full text-sm md:text-base font-medium transition-all duration-300 ${activeTab === 'why' ? 'text-blue-700' : 'text-gray-600 hover:text-blue-600'}`}
                onClick={() => setActiveTab('why')}
              >
                {translations[language].whyTab}
              </button>
              <button 
                className={`relative z-10 px-8 py-3 rounded-full text-sm md:text-base font-medium transition-all duration-300 ${activeTab === 'how' ? 'text-blue-700' : 'text-gray-600 hover:text-blue-600'}`}
        ...
        <div className={`transition-all duration-700 ${activeTab === 'why' ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-32 absolute pointer-events-none'}`}>
          ...
          <div 
            ref={ecosystemRef}
            className={`relative h-96 md:h-128 mb-16 transition-all duration-1000 ${isVisible ? 'scale-100 opacity-100' : 'scale-90 opacity-0'}`}
          >
            {/* SVG for connections */}
            <svg 
              ref={svgRef}
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              ...
              <div className="absolute w-full h-full flex items-center justify-center">
                <div className="absolute w-3/4 h-3/4 rounded-full border border-blue-100 opacity-30 animate-spin-slow"></div>
                <div className="absolute w-2/3 h-2/3 rounded-full border border-purple-100 opacity-30 animate-spin-reverse"></div>
                <div className="absolute w-1/2 h-1/2 rounded-full border border-indigo-100 opacity-30 animate-spin-med"></div>
              </div>
              
              {/* Center Platform Node */}
              <div 
                ref={centerNodeRef} 
                className="relative w-32 h-32 md:w-40 md:h-40 flex items-center justify-center z-20"
              >
                {/* Animated pulse rings */}
                <div className="absolute w-full h-full rounded-full bg-gradient-to-r from-blue-400 to-purple-400 opacity-10 animate-pulse-slow"></div>
                <div className="absolute w-11/12 h-11/12 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 opacity-20 animate-pulse-medium"></div>
                <div className="absolute w-5/6 h-5/6 rounded-full bg-gradient-to-r from-blue-600 to-purple-600 opacity-30 animate-pulse-fast"></div>
                
                {/* Center platform node */}
                <div className="relative w-3/4 h-3/4 rounded-full bg-gradient-to-br from-blue-600 to-purple-700 flex items-center justify-center text-white font-bold text-lg md:text-xl shadow-xl hover:shadow-2xl transition-all duration-500 hover:scale-105 cursor-pointer group">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="relative w-full h-full max-w-xl max-h-xl">
                  {translations[language].entities.map((entity, index) => {
                    ...
                    const radius = '42%'; // Responsive radius
                    ...
                    return (
                      <div
                        ...
                        onClick={() => setActiveEntity(prevActive => prevActive === index ? null : index)}
                      >
                        {/* Entity bubble with modern glass effect */}
                        <div 
                          className={`w-24 h-24 md:w-28 md:h-28 rounded-full ${entity.color} flex items-center justify-center text-white font-semibold cursor-pointer shadow-lg transition-all duration-300 group`}
                          style={{
                            boxShadow: isActive 
                        ...
                        >
                          <div className="flex flex-col items-center">
                            ...
                            {isActive && (
                              <Sparkles className="h-5 w-5 mt-1 text-white opacity-80" />
                            )}
                          </div>
                        ...
                        {/* Benefit card with glass morphism effect */}
                        {isActive && (
                          <div 
                            className="absolute z-40 w-64 benefit-card transform transition-all duration-500 scale-in-center"
                            style={{
                                ...
                                <ul className="space-y-2">
                                  {entity.benefits.map((benefit, i) => (
                                    <li key={i} className="flex items-start benefit-item" style={{ '--benefit-delay': `${i * 150}ms` }}>
                                      <CheckCircle className={`${entity.iconColor} h-5 w-5 mr-2 mt-0.5 flex-shrink-0`} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
            {translations[language].features.map((feature, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`group bg-white rounded-xl border border-gray-200 overflow-hidden transition-all duration-500 transform ${isVisible ? 'opacity-100 translate-y-0 rotate-0' : 'opacity-0 translate-y-20 rotate-2'} shadow-feature-card hover:shadow-feature-glow`}
                  style={{ 
                ...
                >
                  ...
                  <div className="p-6">
                    <div className="p-4 mb-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl w-16 h-16 flex items-center justify-center group-hover:from-blue-100 group-hover:to-indigo-100 transition-colors duration-300 shadow-inner">
                      {React.cloneElement(feature.icon, { size: 32, className: 'text-blue-600' })}
                    </div>
      ...
      <div className="absolute bottom-0 left-0 w-80 h-80 bg-gradient-to-t from-purple-100 to-transparent rounded-full filter blur-3xl opacity-30 transform -translate-x-1/3 translate-y-1/4"></div>
      ...
      <style jsx>{`
        ...
        .animate-spin-med {
          animation: spin-med 60s linear infinite;
        }
        ...
        .animate-dash-fast {
          animation: dash-fast 15s linear infinite;
        }
        .particle {
        ...
        .shadow-feature-glow {
          box-shadow: 0 20px 30px -15px rgba(79, 70, 229, 0.2), 0 10px 20px -10px rgba(79, 70, 229, 0.1);
        }
      `}</style>
    </section>
  );
