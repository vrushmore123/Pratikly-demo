...
const useCases = [
  ...
  },
  {
    ...
    image: "https://images.unsplash.com/photo-1543269865-cbf427effbad?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  ...
  },
  {
    ...
    image: "https://miro.medium.com/v2/resize:fit:1400/1*NQgINaLXYMzvowRHHa6Plw.jpeg",
  ...
  },
  {
    ...
    image: "https://img.freepik.com/free-photo/dynamic-data-visualization-3d_23-2151904311.jpg?semt=ais_hybrid&w=740",
  ...
  },
  {
    ...
    image: "https://www.infosectrain.com/wp-content/themes/Divi/assets/images/banner-placement-assistance-support.png",
...
const UseCases = () => {
  ...
  return (
    <section className={`px-4 py-24 sm:px-8 lg:px-16 xl:px-20 relative overflow-hidden ${
      darkMode 
        ? 'bg-gradient-to-br from-gray-900 to-gray-800' 
        : 'bg-gradient-to-br from-white to-gray-50'
    }`}>
      {/* Enhanced animated background - updated for dark mode */}
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
        <div className={`absolute top-20 -left-20 w-96 h-96 rounded-full ${
          darkMode ? 'bg-emerald-900/10' : 'bg-emerald-100/10'
        } blur-3xl animate-pulse`}></div>
        <div className={`absolute -bottom-32 -right-32 w-96 h-96 rounded-full ${
          darkMode ? 'bg-amber-900/10' : 'bg-amber-100/10'
        } blur-3xl animate-pulse`} style={{animationDelay: '1.5s'}}></div>
        <div className={`absolute top-1/3 right-1/4 w-64 h-64 rounded-full ${
          darkMode ? 'bg-blue-900/10' : 'bg-blue-100/10'
        } blur-3xl animate-pulse`} style={{animationDelay: '0.7s'}}></div>
        
        {/* Floating particles - updated for dark mode */}
        <div className="absolute top-0 left-0 w-full h-full">
          <div className={`absolute top-1/4 left-1/4 w-2 h-2 ${
            darkMode ? 'bg-emerald-600/30' : 'bg-emerald-400/30'
          } rounded-full animate-ping`} style={{animationDuration: '4s'}}></div>
          <div className={`absolute top-1/3 right-1/3 w-3 h-3 ${
            darkMode ? 'bg-amber-600/30' : 'bg-amber-400/30'
          } rounded-full animate-ping`} style={{animationDuration: '5s', animationDelay: '1s'}}></div>
          <div className={`absolute bottom-1/4 left-1/2 w-2 h-2 ${
            darkMode ? 'bg-blue-600/30' : 'bg-blue-400/30'
          } rounded-full animate-ping`} style={{animationDuration: '3.5s', animationDelay: '0.5s'}}></div>
          <div className={`absolute top-2/3 right-1/4 w-2 h-2 ${
            darkMode ? 'bg-purple-600/30' : 'bg-purple-400/30'
          } rounded-full animate-ping`} style={{animationDuration: '4.2s', animationDelay: '1.2s'}}></div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-12 xl:gap-16">
          {useCases.map((useCase, index) => (
            ...
            >
              ...
              {/* Card with improved layout - updated for dark mode */}
              <div className={`relative rounded-2xl overflow-hidden h-full ${
                darkMode 
                  ? 'bg-gray-800 border-gray-700' 
                  : 'bg-white border-gray-100'
              } border shadow-lg group-hover:shadow-2xl transition-all duration-500 z-10`}>
                ...
                <div className={`relative h-64 overflow-hidden`}>
                  ...
                  <div className="absolute inset-0 transform group-hover:scale-105 transition-transform duration-700 ease-out">
                    ...
                    <div className={`absolute inset-0 bg-gradient-to-t ${
                      darkMode ? 'from-black/80 via-black/50' : 'from-black/60 via-black/30'
                    } to-transparent opacity-60 group-hover:opacity-40 transition-opacity duration-500`}></div>
                  </div>
                ...
                <div className="p-8 lg:p-10 flex flex-col justify-between">
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div className={`w-16 h-16 rounded-xl flex items-center justify-center ${
                      darkMode 
                        ? `${useCase.iconBgClass.replace('100', '900')} bg-opacity-20` 
                        : useCase.iconBgClass
                    ...
                    <div className={`px-4 py-2 rounded-full ${
                      darkMode 
                        ? `${useCase.bgClass.replace('50', '900')} bg-opacity-20` 
                        : useCase.bgClass
                    } text-sm font-medium flex items-center space-x-1 transition-transform duration-300 group-hover:scale-105`}>
                      <span className={`w-2 h-2 rounded-full bg-gradient-to-r ${useCase.colorClass}`}></span>
                      <span className={darkMode ? "text-gray-200" : "text-gray-800"}>{useCase.iconType}</span>
                    </div>
                  </div>
                  ...
                  <div className="mt-auto">
                    <Link to="/form" className="block w-full">
                      <div className={`relative group overflow-hidden rounded-lg ${
                        darkMode 
                          ? `${useCase.bgClass.replace('50', '900')} bg-opacity-10 border-gray-700` 
                          : `${useCase.bgClass} ${useCase.borderColorClass}`
                        ...
                        <div className={`px-6 py-4 flex items-center justify-between relative z-10 ${
                          darkMode ? 'group-hover:text-white' : 'group-hover:text-white'
